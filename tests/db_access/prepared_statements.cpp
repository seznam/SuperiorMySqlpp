...
go_bandit([](){
    describe("Test stmt", [&](){
        ...
        it("can work with dynamically allocated strings", [&](){
            ...
            {
                auto&& preparedStatement = connection.makePreparedStatement(
                    "INSERT INTO `test_superior_sqlpp`.`dynamicstring` VALUES (?, ?)",
            ...
            {
                auto&& preparedStatement = connection.makePreparedStatement<ResultBindings<int, Nullable<Dynamic<std::string>>>>(
                    "SELECT `int_value`, `mytext` FROM `test_superior_sqlpp`.`dynamicstring` LIMIT 3"
                );
                preparedStatement.execute();
                int id;
                ...
                AssertThat(*htext, Equals(text));

                AssertThat(preparedStatement.fetch(), IsTrue());
                std::tie(id, htext) = preparedStatement.getResult();
                AssertThat(*htext, Equals("aaa"));
            }
        });
    });
});
